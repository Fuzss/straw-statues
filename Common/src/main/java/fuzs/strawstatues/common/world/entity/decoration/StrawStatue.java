package fuzs.strawstatues.common.world.entity.decoration;
...
public class StrawStatue extends Mannequin implements StatueEntity {
    ...
    /**
     * Called from our custom {@link fuzs.strawstatues.common.network.client.ServerboundPickItemFromEntityMessage}.
     * <p>
     * As the pick result is only created on the server, there is otherwise no way to know whether to include data (from
     * holding down the control key on the client).
     * <p>
     * Also, using the custom packet bypasses calling
     * {@link FetchProfileCommand#printForAvatar(CommandSourceStack, Avatar)} in
     * {@link
     * net.minecraft.server.network.ServerGamePacketListenerImpl#handlePickItemFromEntity(ServerboundPickItemFromEntityPacket)}.
     */
    public ItemStack getPickResult(boolean includeData) {
